...
- No module named 'torch._six'
    - 解决方法：
        - pip3 install --upgrade trl deepspeed  (升级 deepspeed，deepspeed 在较旧版本中仍然依赖 torch._six)
        - pip3 install trl==0.9.6
        - pip3 install torch==2.2.1 

- Traceback (most recent call last):
    File "/home/tiger/.local/lib/python3.9/site-packages/httpx/_urlparse.py", line 412, in normalize_port
        port_as_int = int(port)
    ValueError: invalid literal for int() with base 10: ':'
    - 解决方法：
        - pip3 uninstall gradio

- RuntimeError: The server socket has failed to listen on any local network address. The server socket has failed to bind to [::]:28020 (errno: 98 - Address already in use). The server socket has failed to bind to 0.0.0.0:28020 (errno: 98 - Address already in use).
    - 
